            ...
            {selected?.id === c.id && (
              <div className="mt-4 pt-4 border-t border-slate-100 animate-fade-in">
                <div className="flex gap-2">
                  <button
                    onClick={(e) => { e.stopPropagation(); navigate(`/cours/${c.id}`) }}
                    className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition flex items-center justify-center gap-1">
                    <ChevronRight className="w-3 h-3" /> Commencer le cours
                  </button>
                  <button
                    onClick={(e) => e.stopPropagation()}
                    className="flex-1 py-2 bg-slate-100 text-slate-700 rounded-lg text-xs font-medium hover:bg-slate-200 transition flex items-center justify-center gap-1">
